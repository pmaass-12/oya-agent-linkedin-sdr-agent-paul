...
def run(inp):
    ...
    if not UNIPILE_DSN or not UNIPILE_API_KEY or not UNIPILE_ACCOUNT_ID:
        return {"error": "LinkedIn Messaging not connected. Connect LinkedIn in the Channels/Gateways section to use this skill."}

    actions = {
        "list_chats": lambda: do_list_chats(inp.get("limit", 20)),
        "get_chat": lambda: do_get_chat(inp.get("chat_id", "")),
        "read_messages": lambda: do_read_messages(inp.get("chat_id", ""), inp.get("limit", 20)),
        "send_message": lambda: do_send_message(inp.get("chat_id", ""), inp.get("text", "")),
        "start_chat": lambda: do_start_chat(inp.get("attendees_ids", ""), inp.get("text", "")),
    }

    if not action:
        return {"error": "action is required. Available: list_chats, read_messages, send_message, start_chat, get_chat"}
...
# --- Sandbox entry point (reads INPUT_JSON or INPUT_* env vars) ---
...
if __name__ == "__main__":
    ...
    if raw:
        ...
        # Fallback: read individual INPUT_* env vars
        inp = {
            "action": os.environ.get("INPUT_ACTION", ""),
            "chat_id": os.environ.get("INPUT_CHAT_ID", ""),
            "text": os.environ.get("INPUT_TEXT", ""),
            "attendees_ids": os.environ.get("INPUT_ATTENDEES_IDS", ""),
            "limit": int(os.environ.get("INPUT_LIMIT", "20")),
        }
    result = run(inp)
